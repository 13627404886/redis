...
public class ProductController {
    ...
    @GetMapping("{id}")
    public ReturnObject getProductById(@PathVariable("id") Long id, @RequestParam(required = false, defaultValue = "jpa") String type) {
        ReturnObject retObj = null;
        Product product = null;
        if ("manual".equals(type)) {
            logger.debug("getProductByManual: id = {} " ,id);
            product = productService.findProductById_manual(id);
        }  else if ("autoredis".equals(type)) {
            logger.debug("getProductByRedis: id = {} " ,id);
            product = productService.retrieveProductByIDredis(id, true);
        } else if ("auto".equals(type)) {
            logger.debug("getProductByAuto: id = {} " ,id);
    ...
    @GetMapping("")
    public ReturnObject searchProductByName(@RequestParam String name, @RequestParam(required = false, defaultValue = "jpa") String type) {
        ReturnObject retObj = null;
        ...
        if ("manual".equals(type)) {
            ...
            productList = productService.findProductByName_jpa(name, true);
        }
